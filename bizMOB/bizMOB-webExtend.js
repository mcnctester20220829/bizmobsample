 ...
 */
 bizMOBWeb.Network.requestTr = function(arg) {
    ...
    /**
     * mhchoi: Query 값 추가
     * - 디버깅용 (Mock 호출시 파라미터를 query로 붙이던 거에서 아이디어를 착안)
     * - 동일한 전문을 호출하면서 요청 값의 차이를 Network 탭에서 바로 확인하기 위해 추가
     * - 웹에서만 동작. Native 통신에는 없는 로직
     */
    if (arg._sQuery) {
      url += "?" + arg._sQuery;
    }
    ...
    bizMOBWeb.Http.fetch(url, option, timeout)
        .then(function(res) {
            ...
            if (!res.data.header.result) {
                bizMOBCore.Module.logger(bizMOBWeb.Network.serviceName, "requestLogin", "D", "Request Login Error: " + JSON.stringify(res.data));
            }
